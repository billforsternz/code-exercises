...
void split( const std::string &s, std::vector<std::string> &fields, std::vector<size_t> *offsets )
{
    fields.clear();
    if(offsets)
        offsets->clear();
    size_t next = s.find_first_not_of(" \t",0);
    bool more = (next != std::string::npos);
    while( more && s.length()>next)
    {
        size_t offset = s.find_first_of(" \t",next);
        std::string frag;
        if( offset != std::string::npos )
        ...
        if( more )
        {
            next = s.find_first_not_of(" \t",offset+1);
